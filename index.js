...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=553798624472:+55 (37) 9862-4472\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oiin @${num.split('@')[0]}\nMembro novo mama o adm *${mdata.subject}*`
				...
				teks = `ja vai tarde kkkk @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["553798624472@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '553798624472@s.whatsapp.net'
                        /******ApiKey Input******/
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
               client.sendMessage(from, 'Aqui está o número do meu dono : Victor- Wa.me/553798624472 ',MessageType.text, { quoted: mek} )
                break
	case 'hidetag':
                client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                 if (!isUser) return reply(mess.only.daftarB)
                 if (args.length < 1) return reply(`Insira numeros\nExemplo : ${prefix}spamcall 553798624472...`)
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply('Cade o link seu idiota?')
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'audio':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual � a sigla de idioma? ( pt � a sigla do portugu�s pra vc que � burro )', text, {quoted: mek})
                                   if (!isUser) return reply(mess.only.daftarB)
